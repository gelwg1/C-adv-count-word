...
JRB jrb_find_gte_str(JRB n, char *key, int *fnd)
{
  ...
  *fnd = 0;
  if (!ishead(n)) {
    fprintf(stderr, "jrb_find_gte_str called on non-head 0x\n" );
    exit(1);
  }
...
JRB jrb_find_gte_int(JRB n, int ikey, int *fnd)
{
  *fnd = 0;
  if (!ishead(n)) {
    fprintf(stderr, "jrb_find_gte_int called on non-head 0x\n" );
    exit(1);
  }
  if (n->parent == n) return n;
...
JRB jrb_find_gte_dbl(JRB n, double dkey, int *fnd)
{
  *fnd = 0;
  if (!ishead(n)) {
    fprintf(stderr, "jrb_find_gte_dbl called on non-head 0x\n");
    exit(1);
  }
  if (n->parent == n) return n;
...
void jrb_delete_node(JRB n)
{
  ...
  if (isint(n)) {
    fprintf(stderr, "Cannot delete an internal node: 0x\n");
    exit(1);
  }
  if (ishead(n)) {
    fprintf(stderr, "Cannot delete the head of an jrb_tree: 0x\n");
    exit(1);
  }
...
void jrb_print_tree(JRB t, int level)
{
  ...
  if (ishead(t) && t->parent == t) {
    printf("tree 0x is empty\n");
  } else if (ishead(t)) {
    printf("Head: 0x.  Root = 0x\n");
...
int jrb_nblack(JRB n)
{
  int nb;
  if (ishead(n) || isint(n)) {
    fprintf(stderr, "ERROR: jrb_nblack called on a non-external node 0x\n");
    exit(1);
  }
  nb = 0;
  while(!ishead(n)) {
...
int jrb_plength(JRB n)
{
  int pl;
  if (ishead(n) || isint(n)) {
    fprintf(stderr, "ERROR: jrb_plength called on a non-external node 0x\n");
    exit(1);
  }
  pl = 0;
  while(!ishead(n)) {
